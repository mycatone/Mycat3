...
| 维度 | Mycat3 (Java) | AiCAT3 (Go) |
|------|---------------|-------------|
| 定位 | 完整分布式数据库代理 | 轻量多协议网关 |
| SQL 优化 | Calcite + DRDS 物理计划 | 基础解析直发 |
| 分片算法 | 18 种 | 4 种 |
| 事务 | 完整 XA / 2PC | 简化事务 |
| 部署 | JDK 8+, ~150MB JAR | 单文件 39MB 二进制 |
| 启动速度 | 秒级 | 毫秒级 |
| 内存占用 | 数百 MB | 数十 MB |
| 推荐场景 | 分库分表、跨分片 JOIN、强一致事务 | 多协议统一入口 + 简单 key 路由 |
...
- 配置文档：[Mycat3配置说明.md](Mycat3配置说明.md)
